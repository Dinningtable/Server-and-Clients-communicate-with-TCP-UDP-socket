...
const char *M_REGISTER_ERR_USAGE =
    "Usage: register <username> <email> <password>\n";

const char *M_LOGIN_SUCCESS =
    "Welcome, "; // strcat with username and ".\n" for full message
...
// Make this a command processing function with void return value (Take modified
// variables' addresses as input)
void handleCommand(char *msg, int nParams, char params[][MAXLINE],
                   int *loggedInPtr, int *inGamePtr, char *ans,
                   char *userName)
{
  int comType, resultCode;
...
void *routine(void *param) {
  ...
  int inGame = 0;   // 1 or above if in game, 0 if not, indicates number of
                    // guesses made if > 0
  char ans[5] = "1234";
...
void *udpProcessAndSendRoutine(void *param) {
  ...
  sendto(paramPtr->uServerSocketFd, msg, strlen(msg) + 1, 0,
         (struct sockaddr *)&paramPtr->clientAddress,
         paramPtr->clientAddressLength);
...
void *udpRoutine(void *param) {
  ...
  // UDP binding
  if (bind(uServerSocketFd, (struct sockaddr *)&serverAddress,
  ...
  while (1) {
    rMsgLength =
        recvfrom(uServerSocketFd, msg, sizeof(msg), 0,
    ...
    for (int i = 0; i < 100; i++) {
      if (threadAvailable[i] == 1) {
        ...
        pthread_create(&newThread[i], NULL, &udpProcessAndSendRoutine,
                       &uParamsArr[i]);
        break;
      }
    }
